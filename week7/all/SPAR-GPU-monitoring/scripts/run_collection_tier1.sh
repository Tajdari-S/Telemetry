#!/usr/bin/env bash
# Week 3 full data collection — Tier 1 only (pynvml, no DCGM), GPU 0, all workloads, 3 runs each
set -euo pipefail
cd "$(dirname "$0")/.."
...
RUN() {
    ...
    echo "========================================"
    $PYTHON scripts/run_workload.py --workload "$LABEL" --no-dcgm
    echo "--- $LABEL run $RUN_N complete ---"
    sleep 10
}

echo "=== Week 3 Tier-1 collection started at $(date) ==="
